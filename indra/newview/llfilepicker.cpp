...
#define WEARABLES_FILTER L"Wearable (*.shape; *.skin; *.hair; *.eyes; *.shirt; *.pants; *.shoes; *.socks; *.jacket; *.gloves; *.undershirt; *.underpants; *.skirt)\0*.shape; *.skin; *.hair; *.eyes; *.shirt; *.pants; *.shoes; *.socks; *.jacket; *.gloves; *.undershirt; *.underpants; *.skirt\0"
#define NOTECARD_FILTER L"Notecards (*.notecard; *.txt)\0*.notecard; *.txt\0"
#define LSL_FILTER L"LSL (*.lsl; *.txt)\0*.lsl; *.txt\0"
#define XML_FILTER L"XML files (*.xml)\0*.xml\0"
#define LANDMARK_FILTER L"Landmarks (*.Landmark)\0*.Landmark\0"
...
BOOL LLFilePicker::setupFilter(ELoadFilter filter)
{
	...
	case FFLOAD_NOTECARD:
		mOFN.lpstrFilter = NOTECARD_FILTER \
			L"\0";
		break;
	case FFLOAD_WEARABLES:
		mOFN.lpstrFilter = WEARABLES_FILTER \
			L"\0";
		break;
	case FFLOAD_LSL:
		mOFN.lpstrFilter = LSL_FILTER \
			L"\0";
		break;
	case FFLOAD_LANDMARK:
		mOFN.lpstrFilter = LANDMARK_FILTER \
			L"\0";
		break;
	// </edit>
	default:
...
BOOL LLFilePicker::getSaveFile(ESaveFilter filter, const std::string& filename)
{
	...
	case FFSAVE_ANIMATN:
		if (filename.empty())
		{
			wcsncpy( mFilesW,L"untitled.animatn", FILENAME_BUFFER_SIZE);	/*Flawfinder: ignore*/
		}
		mOFN.lpstrDefExt = L"animatn";
		mOFN.lpstrFilter =
			L"ANIMATN Anim File (*.animatn)\0*.animatn\0" \
			L"\0";
		break;	
	case FFSAVE_NEIL:
		if (filename.empty())
		{
			wcsncpy( mFilesW,L"untitled.neil", FILENAME_BUFFER_SIZE);	/*Flawfinder: ignore*/
		}
		mOFN.lpstrDefExt = L"neil";
		mOFN.lpstrFilter =
			L"NEIL Anim File (*.neil)\0*.neil\0" \
			L"\0";
		break;
		case FFSAVE_ANIMS:
		if (filename.empty())
		{
			wcsncpy( mFilesW,L"untitled.anim", FILENAME_BUFFER_SIZE);	/*Flawfinder: ignore*/
		}
		mOFN.lpstrDefExt = L"anim";
		mOFN.lpstrFilter =
			L"ANIMS Anim File (*.anim)\0*.anim\0" \
			L"\0";
		break;	
	case FFSAVE_OGG:
		if (filename.empty())
		{
			wcsncpy( mFilesW,L"untitled.ogg", FILENAME_BUFFER_SIZE);	/*Flawfinder: ignore*/
		}
		mOFN.lpstrDefExt = L"ogg";
		mOFN.lpstrFilter =
			L"OGG Sounds (*.ogg)\0*.ogg\0" \
			L"\0";
		break;
